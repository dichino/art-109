...
let scene, camera, renderer, ball, dog, mixer, actionPant, actionTail;
...
function init() {
    ...
    // ~~ add directional light 
    const lightLeft = new THREE.DirectionalLight(0xffF0000, 3);
    ...
    loader.load('assets/dog_shiny.gltf', function (gltf) {
        dog = gltf.scene; // global variable
        scene.add(dog);
...
function animate() {
    ...
    const delta = clock.getDelta() //get delta one time per frame
    ...
    if (dog) {
        ...
        dog.rotation.y =  Math.sin(Date.now()/1500) * 1.5 ;
    }
